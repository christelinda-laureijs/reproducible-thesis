...
You can embed R code within normal text to create dynamic reports. For example, if you write \` r nrow(plant_data) \`, this will be printed out as `r nrow(plant_data)`, as in "We weighed `r nrow(plant_data)` plants".
...
There are many packages that will allow you to create publication-quality tables. I've found that the packages kable and broom are particularly useful. Let's say that I want to make the summary table look better
...
\\setcounter{tocdepth}{2} to set the table of content depth to header 2
\\tableofcontents to insert a table of contents
