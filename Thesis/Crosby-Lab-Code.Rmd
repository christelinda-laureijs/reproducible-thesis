...
# 5th argument: Include representative trace ("yes" or "no")
# 6th argument: If include_representative_trace == "yes", state the name of the trace imported in the Import-representative-traces chunk
# 7th argument: Set to "no" 
...
```{r plot-HNMPA}
#| fig.dim = c(8,5),
#| fig.cap = "Each point represents the mean ± SE eEPSC amplitude and \\emph{n} represents the number of unique cells. The asterisks indicate a statistically significant decrease in current amplitude relative to the baseline (t-test; * = p < 0.05, ** = p < 0.01, *** = p < 0.001). The representative traces consist of eEPSCs from one cell averaged over the baseline period (0 to 5 min) and last interval (20 to 25 min) of the recording, and the scale bar represents 50 pA/20 ms. \\label{summary-plot-HNMPA}",
#| fig.scap = "HNMPA Summary Plot",
#| out.width = "100%"
...
```{r plot-PPP}
#| fig.dim = c(8,5),
#| fig.cap = "Each point represents the mean ± SE eEPSC amplitude and \\emph{n} represents the number of unique cells. There are no asterisks because the decrease in eEPSC amplitude was not statistically significant relative to the baseline. The representative traces consist of eEPSCs from one cell averaged over the baseline period (0 to 5 min) and last interval (20 to 25 min) of the recording, and the scale bar represents 50 pA/20 ms. \\label{summary-plot-PPP}",
#| fig.scap = "PPP Summary Plot",
#| out.width = "100%"
make_pruned_summary_plots(category = 2,
                         treatment = "PPP",
                         data = pruned_df,
...
#| fig.cap = "Each neuron is coloured according to the relative decrease in eEPSC amplitude after 20 minutes of insulin exposure. Neurons with similar responses to insulin did not cluster together, and there were no regions within the DMH that had a greater proportion of highly responsive neurons. In this figure (magnified 5X), the top of the third ventricle is located at (0,0). \\label{cell-locations}",
...
# Final Treatment Comparisons Plot

```{r treatment-comparisons-plot}
#| fig.cap = "In this summary figure, each dot represents the mean eEPSC amplitude of a cell relative to the baseline (%) during the last 20-25 minutes of the evoked currents protocol. Black solid shapes indicate the mean ± SE for each sex. \\label{eEPSC-treatment-comparisons-plot}",
#| fig.scap = "Treatment Comparisons Plot",
#| out.width = "100%"
...
#knitr::purl(input = here::here("Thesis/Crosby-Lab-Code.Rmd"), output = here::here("Scripts/Scripts-from-Crosby-Lab-Code-Rmd.R"), documentation = 0)
```
